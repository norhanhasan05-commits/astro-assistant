...
app = FastAPI(title="Astro Assistant", version="1.8")
...
def comparison_metrics(result: dict) -> dict:
    sources = [
        ("العبور", clean_analysis_aspects(result["transits"]["aspects_to_natal"], "العبور"), 1.20),
        ("العودة الشمسية", clean_analysis_aspects(result["solar_return"]["aspects_to_natal"], "العودة الشمسية"), 0.90),
        ("التقدمات", clean_analysis_aspects(result["secondary_progressions"]["aspects_to_natal"], "التقدمات الثانوية"), 1.00),
    ]
    supportive, hard, conjunctions, top = [], [], [], []
    for source_name, aspects, source_weight in sources:
        for a in aspects:
            item = dict(a)
            item["_source"] = source_name
            item["_weighted_strength"] = strength_score(a) * source_weight
            top.append(item)
            if a["aspect"] in ("Trine", "Sextile"):
                supportive.append(item["_weighted_strength"])
            elif a["aspect"] in ("Square", "Opposition"):
                hard.append(item["_weighted_strength"])
            elif a["aspect"] == "Conjunction":
                conjunctions.append(item["_weighted_strength"])
    total_relevant = len(supportive) + len(hard) + len(conjunctions)
    if total_relevant == 0:
        index = 50.0
    else:
        support_ratio = (len(supportive)-len(hard))/max(1,len(supportive)+len(hard))
        support_avg = sum(supportive)/len(supportive) if supportive else 0
        hard_avg = sum(hard)/len(hard) if hard else 0
        index = 50 + support_ratio*24 + (support_avg-hard_avg)*0.12 + min(6.0,len(conjunctions)*0.45)
        index = max(10.0,min(90.0,index))
    ranked=sorted(top,key=lambda a:(-a["_weighted_strength"],float(a.get("orb",9))))[:5]
    return {"index":round(index,1),"supportive":len(supportive),"hard":len(hard),"conjunctions":len(conjunctions),"total":total_relevant,"top":ranked}
...
def comparison_top_table(metrics: dict) -> str:
    ...
    for a in metrics["top"]:
        ...
        rows.append(f"<tr><td>{html.escape(a['_source'])}</td><td>{html.escape(title)}</td><td>{strength_score(a)}/100</td><td>{float(a['orb']):.2f}°</td></tr>")
    return table_wrap("<tr><th>المصدر</th><th>الإشارة</th><th>القوة</th><th>الأورب</th></tr>","".join(rows))
...
def comparison_summary(team1: str,m1: dict,team2: str,m2: dict) -> str:
    ...
    if diff<4:
        verdict="المؤشرات متقاربة جدًا بين الطرفين."
    elif m1["index"]>m2["index"]:
        verdict=f"المؤشر الداخلي يميل نسبيًا إلى {team1}."
    else:
        verdict=f"المؤشر الداخلي يميل نسبيًا إلى {team2}."
    return f'<section class="result-card highlight"><h2>نتيجة المقارنة</h2><div class="comparison-score"><div><span>{html.escape(team1)}</span><strong>{m1["index"]}/100</strong><small>حصة المؤشر: {share1}%</small></div><div class="versus">VS</div><div><span>{html.escape(team2)}</span><strong>{m2["index"]}/100</strong><small>حصة المؤشر: {share2}%</small></div></div><div class="climate climate-neutral">{html.escape(verdict)}</div><p class="muted">هذه ليست نسبة فوز، بل مقارنة داخلية بين الإشارات الرمزية التي حسبها النظام.</p></section>'
...
</style></head><body><div class="wrap"><a class="back" href="/">← العودة للتحليل الفردي</a><div class="hero"><h1>⚔️ مقارنة مباراة</h1><p>استخدم نفس وقت ومكان المباراة للطرفين.</p></div>
