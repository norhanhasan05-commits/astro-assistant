...
app = FastAPI(title="Astro Assistant", version="1.6")
...
def chart_wheel(
    ...
    if aspects:
        for a in aspects[:80]:
            ...
            # In overlay charts, moving factors belong to the overlay ring.
    ...
    for name, data in planets.items():
        ...
        svg.append(
            f"<text x='{px:.2f}' y='{py:.2f}' class='planet-label'>{html.escape(label + retro)}</text>"
        )
    ...
    if overlay:
        ...
        for name, data in overlay.items():
            ...
            svg.append(
                f"<text x='{px:.2f}' y='{py:.2f}' class='overlay-label'>{html.escape(label + retro)}</text>"
            )
