...
from astro_core import calculate, local_to_utc, dt_to_jd, planets, houses, factors, angular_distance, normalize, ASPECTS

app = FastAPI(title="Astro Assistant", version="2.5.1")
...
ASPECT_AR = {
    ...
    "Trine":"تثليث",
    "Opposition":"مقابلة",
    "Semisextile":"نصف تسديس",
    "Semisquare":"نصف تربيع",
    "Sesquisquare":"تربيع ونصف",
    "Quincunx":"كوينكونكس",
}
...
def primary_table(rows: list) -> str:
    if not rows:
        return "<p class='muted'>لا توجد توجيهات أولية قريبة ضمن الأورب الدقيق.</p>"
    out = []
    for a in rows[:120]:
        asp = str(a["aspect"])
        css = ASPECT_CLASS.get(asp, "")
        out.append(
            ...
            f"<td>{html.escape(AR_PLANETS.get(a['significator'], a['significator']))}</td>"
            f"<td>{html.escape(ASPECT_AR.get(asp, asp))}</td>"
            f"<td>{html.escape(AR_PLANETS.get(a['natal_factor'], a['natal_factor']))}</td>"
            f"<td>{float(a['orb_arcmin']):.2f}′</td>"
            f"<td>±{float(a.get('time_window_days', 0)):.1f} يوم</td></tr>"
        )
    return table_wrap(
        "<tr><th>الاتجاه</th><th>العامل الموجّه</th><th>الزاوية</th>"
        "<th>العامل الميلادي</th><th>الأورب</th><th>نافذة التقريب</th></tr>",
        "".join(out)
    )
...
def render_results(result: dict) -> str:
    ...
    blocks.append(section(
        "التوجيهات الأولية الموسعة — Beta",
        primary_table(result["primary_directions_experimental"]) +
        "<p class='muted'>تشمل الكواكب والعقدة وPart of Fortune وASC/MC ورؤوس البيوت، مباشرًا وعكسيًا، مع زوايا كبرى وصغرى وأورب بالدقائق القوسية. هذه نسخة Zodiacal Beta وليست بعدُ Speculum توبوسنتري كاملًا.</p>",
        "Primary Directions"
    ))
...
<div class="wrap">
  ...
  <div class="card">
    ...
    <div class="module-grid">
      ...
      <a class="module-link" href="/monthly">🗓️ التحليل الشهري المتقدم</a>
      <a class="module-link" href="/relationships">❤️ تحليل العلاقات</a>
      <a class="module-link" href="/electional">🎯 اختيار أفضل موعد</a>
      <a class="module-link" href="/match">⚔️ مقارنة مباراة</a>
      <a class="module-link" href="#results">🧭 التوجيهات الأولية الموسعة — Beta</a>
    </div>
  </div>
  ...
  <form method="post" action="/analyze-form">
    <div class="card">
      ...
      <label>تاريخ ووقت الميلاد</label><input name="birth_datetime" value="1994-05-24T03:40:00">
      <div class="grid">
...
</style></head><body><div class="wrap"><a class="back" href="/">← العودة للتحليل الفردي</a> <a class="back" href="/match-full">⚽ افتح المحرك المركب</a><div class="hero"><h1>⚔️ مقارنة مباراة — المحرك المتقدم</h1><p>أوزان مختلفة للعبور والعودة الشمسية والتقدمات والزوايا الدقيقة والتكرار بين التقنيات.</p></div>
...
def full_match_side_score(
    coach_metrics: dict,
    captain_metrics: dict,
    coach_weight: float,
    captain_weight: float,
    coach_time_quality: float,
    captain_time_quality: float,
) -> dict:
    total_weight = coach_weight + captain_weight
    if total_weight <= 0:
        coach_weight, captain_weight, total_weight = 40.0, 60.0, 100.0

    score = (
        float(coach_metrics["index"]) * coach_weight
        + float(captain_metrics["index"]) * captain_weight
    ) / total_weight

    # جودة وقت الميلاد تؤثر في الثقة فقط، وليس في المؤشر الرمزي نفسه.
    confidence = (
        float(coach_metrics["confidence"]) * coach_weight * coach_time_quality
        + float(captain_metrics["confidence"]) * captain_weight * captain_time_quality
    ) / total_weight

    return {
        "score": round(score, 1),
        "confidence": round(max(10.0, min(95.0, confidence)), 1),
        "coach_score": float(coach_metrics["index"]),
        "captain_score": float(captain_metrics["index"]),
        "coach_weight": coach_weight,
        "captain_weight": captain_weight,
    }


def full_match_summary(
    team1: str,
    side1: dict,
    team2: str,
    side2: dict,
) -> str:
    total = side1["score"] + side2["score"]
    share1 = round(side1["score"] / total * 100, 1) if total else 50.0
    share2 = round(100 - share1, 1)
    diff = round(abs(side1["score"] - side2["score"]), 1)

    if diff < 2:
        verdict = "النتيجة شديدة التقارب؛ لا توجد أفضلية واضحة داخل النموذج."
        strength = "متعادلة تقريبًا"
    elif diff < 5:
        leader = team1 if side1["score"] > side2["score"] else team2
        verdict = f"أفضلية رمزية طفيفة لصالح {leader}."
        strength = "طفيفة"
    elif diff < 10:
        leader = team1 if side1["score"] > side2["score"] else team2
    ...
    confidence = round((side1["confidence"] + side2["confidence"]) / 2, 1)

    return (
        '<section class="result-card highlight">'
        '<h2>النتيجة المركبة للمباراة</h2>'
        '<div class="comparison-score">'
        f'<div><span>{html.escape(team1)}</span><strong>{side1["score"]}/100</strong>'
        f'<small>حصة المؤشر: {share1}%</small></div>'
        '<div class="versus">VS</div>'
        f'<div><span>{html.escape(team2)}</span><strong>{side2["score"]}/100</strong>'
        f'<small>حصة المؤشر: {share2}%</small></div>'
        '</div>'
        f'<div class="climate climate-neutral">{html.escape(verdict)}</div>'
        '<div class="stats">'
        f'<div><span>قوة الأفضلية</span><strong>{strength}</strong></div>'
        f'<div><span>فرق المؤشر</span><strong>{diff}</strong></div>'
        f'<div><span>ثقة البيانات</span><strong>{confidence}%</strong></div>'
        '<div><span>التركيب</span><strong>مدرب + قائد</strong></div>'
        '</div>'
        '<p class="muted">هذه مقارنة رمزية داخلية وليست احتمال فوز علميًا. جودة وقت الميلاد تؤثر في درجة الثقة فقط.</p>'
        '</section>'
    )


def full_match_layers_table(
    team1: str,
    coach1: dict,
    captain1: dict,
    team2: str,
    ...
    rows = [
        ...
        (
            "القائد",
            float(captain1["index"]),
            float(captain2["index"]),
            captain_weight,
        ),
    ]
    html_rows = []
    for layer, v1, v2, weight in rows:
        better = team1 if v1 > v2 else team2 if v2 > v1 else "متعادل"
        html_rows.append(
            f"<tr><td>{html.escape(layer)}</td><td>{weight:.0f}%</td>"
            f"<td>{v1:.1f}</td><td>{v2:.1f}</td><td>{html.escape(better)}</td></tr>"
        )
    return table_wrap(
        f"<tr><th>الطبقة</th><th>الوزن</th><th>{html.escape(team1)}</th>"
        f"<th>{html.escape(team2)}</th><th>الأفضل</th></tr>",
        "".join(html_rows),
    )
...
<meta charset="utf-8">
<meta name="viewport" content="width=device-width,initial-scale=1">
<title>محرك المباراة المركب</title>
<style>
:root{--bg:#0e1117;--panel:#171c24;--panel2:#11161d;--border:#2c3440;--text:#f4f6f8;--muted:#aeb8c6;--accent:#7c5cff;--accent2:#9b86ff}
*{box-sizing:border-box}body{font-family:Arial,Tahoma,sans-serif;background:var(--bg);color:var(--text);margin:0}
.wrap{max-width:1120px;margin:auto;padding:16px}.hero{text-align:center;padding:12px 0}
.card,.result-card{background:var(--panel);border:1px solid var(--border);border-radius:18px;padding:18px;margin:14px 0}
h1,h2,h3{margin-top:0}h3{color:#ddd7ff;margin-top:18px}
label{display:block;margin:10px 0 5px;color:#d2d8e2}
input,select{width:100%;padding:12px;border-radius:10px;border:1px solid #3a4453;background:var(--panel2);color:white;font-size:15px}
...
.grid{display:grid;grid-template-columns:repeat(3,1fr);gap:10px}.sides{display:grid;grid-template-columns:1fr 1fr;gap:14px}
.person{background:var(--panel2);border:1px solid var(--border);border-radius:14px;padding:14px;margin-top:12px}
.table-wrap{overflow-x:auto;border:1px solid var(--border);border-radius:12px}table{width:100%;border-collapse:collapse;min-width:650px;background:var(--panel2)}
th,td{padding:11px;border-bottom:1px solid var(--border);text-align:right}th{background:#191f29}
.highlight{border-color:#6f59db}.comparison-score{display:grid;grid-template-columns:1fr auto 1fr;gap:16px;align-items:center;margin:18px 0}
.comparison-score>div:not(.versus){background:var(--panel2);border-radius:14px;padding:18px;text-align:center}
.comparison-score span,.comparison-score small{display:block;color:var(--muted)}.comparison-score strong{display:block;font-size:34px;margin:8px 0}
.versus{font-weight:bold;color:#9b86ff}.climate{padding:13px;border-radius:12px;margin-bottom:10px}.climate-neutral{background:#2d3040;color:#d6d8ff}
.stats{display:grid;grid-template-columns:repeat(4,1fr);gap:10px}.stats div{background:var(--panel2);padding:12px;border-radius:12px}
.stats span{display:block;color:var(--muted);font-size:13px}.stats strong{font-size:20px}.muted{color:var(--muted);font-size:13px}.back{color:#cbbfff;text-decoration:none}
@media(max-width:760px){.grid,.sides,.comparison-score,.stats{grid-template-columns:1fr}.versus{text-align:center}}
</style>
</head>
<body><div class="wrap">
<a class="back" href="/match">← المقارنة الفردية</a>
<div class="hero"><h1>⚽ محرك المباراة المركب</h1>
<p class="muted">يجمع مقارنة المدربين ومقارنة القائدين في نتيجة واحدة قابلة لتعديل الأوزان.</p></div>

<form method="post" action="/match-full-compare">
<div class="sides">

<section class="card">
<h2>🇦🇷 الطرف الأول</h2>
<label>اسم المنتخب</label><input name="team1" value="Argentina">

<div class="person"><h3>المدرب</h3>
<label>الاسم</label><input name="coach1_name" value="Lionel Scaloni">
<label>تاريخ ووقت الميلاد</label><input name="coach1_birth" value="1978-05-16T12:00:00">
<div class="grid"><div><label>UTC</label><input name="coach1_offset" value="-3"></div>
<div><label>خط العرض</label><input name="coach1_lat" value="-33.01"></div>
<div><label>خط الطول</label><input name="coach1_lon" value="-61.04"></div></div>
<div class="grid"><div><label>عرض العودة</label><input name="coach1_sr_lat" value="-33.01"></div>
<div><label>طول العودة</label><input name="coach1_sr_lon" value="-61.04"></div>
<div><label>جودة وقت الميلاد</label><select name="coach1_quality"><option value="0.65">غير موثق</option><option value="1">موثق</option></select></div></div>
</div>

<div class="person"><h3>القائد</h3>
<label>الاسم</label><input name="captain1_name" value="Lionel Messi">
<label>تاريخ ووقت الميلاد</label><input name="captain1_birth" value="1987-06-24T06:10:00">
<div class="grid"><div><label>UTC</label><input name="captain1_offset" value="-3"></div>
<div><label>خط العرض</label><input name="captain1_lat" value="-32.95"></div>
<div><label>خط الطول</label><input name="captain1_lon" value="-60.67"></div></div>
<div class="grid"><div><label>عرض العودة</label><input name="captain1_sr_lat" value="-32.95"></div>
<div><label>طول العودة</label><input name="captain1_sr_lon" value="-60.67"></div>
<div><label>جودة وقت الميلاد</label><select name="captain1_quality"><option value="1">موثق/متداول</option><option value="0.65">غير موثق</option></select></div></div>
</div>
</section>

<section class="card">
<h2>🏴 الطرف الثاني</h2>
<label>اسم المنتخب</label><input name="team2" value="England">

<div class="person"><h3>المدرب</h3>
<label>الاسم</label><input name="coach2_name" value="Thomas Tuchel">
<label>تاريخ ووقت الميلاد</label><input name="coach2_birth" value="1973-08-29T12:00:00">
<div class="grid"><div><label>UTC</label><input name="coach2_offset" value="2"></div>
<div><label>خط العرض</label><input name="coach2_lat" value="48.25"></div>
<div><label>خط الطول</label><input name="coach2_lon" value="10.37"></div></div>
<div class="grid"><div><label>عرض العودة</label><input name="coach2_sr_lat" value="48.25"></div>
<div><label>طول العودة</label><input name="coach2_sr_lon" value="10.37"></div>
<div><label>جودة وقت الميلاد</label><select name="coach2_quality"><option value="0.65">غير موثق</option><option value="1">موثق</option></select></div></div>
</div>

<div class="person"><h3>القائد</h3>
<label>الاسم</label><input name="captain2_name" value="Harry Kane">
<label>تاريخ ووقت الميلاد</label><input name="captain2_birth" value="1993-07-28T12:00:00">
<div class="grid"><div><label>UTC</label><input name="captain2_offset" value="1"></div>
<div><label>خط العرض</label><input name="captain2_lat" value="51.59"></div>
<div><label>خط الطول</label><input name="captain2_lon" value="-0.02"></div></div>
<div class="grid"><div><label>عرض العودة</label><input name="captain2_sr_lat" value="51.59"></div>
<div><label>طول العودة</label><input name="captain2_sr_lon" value="-0.02"></div>
<div><label>جودة وقت الميلاد</label><select name="captain2_quality"><option value="0.65">غير موثق</option><option value="1">موثق</option></select></div></div>
</div>
</section>
</div>
...
<label>تاريخ ووقت البداية المحلي في مكان المباراة</label>
<input name="event_datetime" value="2026-07-15T15:00:00">
<div class="grid"><div><label>UTC مكان المباراة</label><input name="event_offset" value="-4"></div>
<div><label>خط العرض</label><input name="event_lat" value="33.7554"></div>
<div><label>خط الطول</label><input name="event_lon" value="-84.4009"></div></div>
<div class="grid"><div><label>وزن المدرب %</label><input name="coach_weight" value="40"></div>
<div><label>وزن القائد %</label><input name="captain_weight" value="60"></div>
<div><label>الأورب الأقصى</label><input name="max_orb" value="2.0"></div></div>
<button type="submit">احسب النتيجة المركبة</button>
</section>
</form>
__FULL_RESULT__
<p class="muted">ملاحظة: لم نضف الدولة أو اتحاد المنتخب أو خريطة البداية كطرف فائز؛ لأن إسناد خريطة الحدث لأحد الجانبين يحتاج قواعد رياضية فلكية مستقلة واختبارات موثوقة.</p>
</div></body></html>
"""


@app.get("/match-full", response_class=HTMLResponse)
def match_full_page():
    return FULL_MATCH_PAGE.replace("__FULL_RESULT__", "")


@app.post("/match-full-compare", response_class=HTMLResponse)
def match_full_compare(
    team1: str = Form(...),
    coach1_name: str = Form(...),
    coach1_birth: str = Form(...),
    coach1_offset: float = Form(...),
    coach1_lat: float = Form(...),
    coach1_lon: float = Form(...),
    coach1_sr_lat: float = Form(...),
    coach1_sr_lon: float = Form(...),
    coach1_quality: float = Form(0.65),
    captain1_name: str = Form(...),
    captain1_birth: str = Form(...),
    captain1_offset: float = Form(...),
    captain1_lat: float = Form(...),
    captain1_lon: float = Form(...),
    captain1_sr_lat: float = Form(...),
    captain1_sr_lon: float = Form(...),
    captain1_quality: float = Form(1.0),

    team2: str = Form(...),
    coach2_name: str = Form(...),
    coach2_birth: str = Form(...),
    coach2_offset: float = Form(...),
    coach2_lat: float = Form(...),
    coach2_lon: float = Form(...),
    coach2_sr_lat: float = Form(...),
    coach2_sr_lon: float = Form(...),
    coach2_quality: float = Form(0.65),
    captain2_name: str = Form(...),
    captain2_birth: str = Form(...),
    captain2_offset: float = Form(...),
    captain2_lat: float = Form(...),
    captain2_lon: float = Form(...),
    captain2_sr_lat: float = Form(...),
    captain2_sr_lon: float = Form(...),
    captain2_quality: float = Form(0.65),

    event_datetime: str = Form(...),
    event_offset: float = Form(...),
    event_lat: float = Form(...),
    event_lon: float = Form(...),
    coach_weight: float = Form(40),
    captain_weight: float = Form(60),
    ...
    try:
        ...
        payloads = [
            full_match_person_payload(common, coach1_name, coach1_birth, coach1_offset, coach1_lat, coach1_lon, coach1_sr_lat, coach1_sr_lon),
            full_match_person_payload(common, captain1_name, captain1_birth, captain1_offset, captain1_lat, captain1_lon, captain1_sr_lat, captain1_sr_lon),
            full_match_person_payload(common, coach2_name, coach2_birth, coach2_offset, coach2_lat, coach2_lon, coach2_sr_lat, coach2_sr_lon),
            full_match_person_payload(common, captain2_name, captain2_birth, captain2_offset, captain2_lat, captain2_lon, captain2_sr_lat, captain2_sr_lon),
        ]
        results = [calculate(p) for p in payloads]
        coach1_m, captain1_m, coach2_m, captain2_m = [comparison_metrics(r) for r in results]

        side1 = full_match_side_score(
            coach1_m, captain1_m, coach_weight, captain_weight,
            coach1_quality, captain1_quality,
        )
        side2 = full_match_side_score(
            coach2_m, captain2_m, coach_weight, captain_weight,
            coach2_quality, captain2_quality,
        )

        output = full_match_summary(team1, side1, team2, side2)
        output += (
            '<section class="result-card"><h2>تفصيل الطبقات</h2>'
            + full_match_layers_table(
                team1, coach1_m, captain1_m,
                team2, coach2_m, captain2_m,
                coach_weight, captain_weight,
            )
            + '</section>'
        )

        detail_people = [
            (team1, "المدرب", coach1_name, coach1_m, coach1_quality),
            (team1, "القائد", captain1_name, captain1_m, captain1_quality),
            (team2, "المدرب", coach2_name, coach2_m, coach2_quality),
            (team2, "القائد", captain2_name, captain2_m, captain2_quality),
        ]
        for team, role, person, metrics, quality in detail_people:
            quality_label = "موثق/متداول" if quality >= 0.9 else "غير موثق"
            output += (
                '<section class="result-card">'
                f'<h2>{html.escape(team)} — {html.escape(role)}: {html.escape(person)}</h2>'
                '<div class="stats">'
                f'<div><span>المؤشر</span><strong>{metrics["index"]}/100</strong></div>'
                f'<div><span>وقت الميلاد</span><strong>{quality_label}</strong></div>'
                f'<div><span>مساندة</span><strong>{metrics["supportive"]}</strong></div>'
                f'<div><span>ضاغطة</span><strong>{metrics["hard"]}</strong></div>'
                '</div>'
                '<h3>أقوى الإشارات</h3>'
                f'{comparison_top_table(metrics)}'
                '</section>'
            )

    except Exception as exc:
        output = (
            '<section class="result-card"><h2>حدث خطأ في الحساب المركب</h2>'
            f'<pre>{html.escape(str(exc))}</pre></section>'
        )

    return FULL_MATCH_PAGE.replace("__FULL_RESULT__", output)



RELATION_ASPECT_POLARITY = {
    "Conjunction": 0.25,
    "Sextile": 0.70,
    "Square": -0.85,
    "Trine": 1.00,
...
RELATION_CATEGORIES = {
    ...
    "mental": {"Mercury", "Sun", "Uranus"},
    "attraction": {"Venus", "Mars", "Pluto", "ASC"},
    "stability": {"Saturn", "Jupiter", "Sun", "Moon"},
}


def _planet_pair_weight(a: str, b: str) -> float:
    return max(
        RELATION_PLANET_WEIGHTS.get(a, 0.75),
        RELATION_PLANET_WEIGHTS.get(b, 0.75),
    )


def _relationship_rows(f1: dict, f2: dict, max_orb: float = 4.0) -> list:
    rows = []
    for name1, lon1 in f1.items():
        for name2, lon2 in f2.items():
            sep = angular_distance(float(lon1), float(lon2))
            for aspect_name, exact in ASPECTS.items():
                orb = abs(sep - exact)
                if orb <= max_orb:
                    precision = max(0.15, 1.0 - orb / max_orb)
                    polarity = RELATION_ASPECT_POLARITY.get(aspect_name, 0.0)
                    weight = _planet_pair_weight(name1, name2)
                    score = 10.0 * precision * polarity * weight
                    rows.append({
                        "factor1": name1,
                        "factor2": name2,
                        "aspect": aspect_name,
                    ...
                    break
    return sorted(rows, key=lambda x: (-abs(x["score"]), x["orb"]))


def _category_score(rows: list, category: str) -> float:
    category_planets = RELATION_CATEGORIES[category]
    relevant = [
        r for r in rows
        if r["factor1"] in category_planets or r["factor2"] in category_planets
    ]
    if not relevant:
        return 50.0
    net = sum(r["score"] for r in relevant)
    scale = max(16.0, len(relevant) ** 0.5 * 8.0)
    return round(max(5.0, min(95.0, 50 + 42 * math.tanh(net / scale))), 1)
...
def _midpoint_angle(a: float, b: float) -> float:
    ...
    return normalize(a + diff / 2)


def _composite_chart(f1: dict, f2: dict) -> dict:
    shared = sorted(set(f1).intersection(f2))
    return {name: round(_midpoint_angle(f1[name], f2[name]), 6) for name in shared}


def _davison_chart(
    birth1_local: str, offset1: float, lat1: float, lon1: float,
    birth2_local: str, offset2: float, lat2: float, lon2: float,
) -> dict:
    dt1 = local_to_utc(datetime.fromisoformat(birth1_local), offset1)
    dt2 = local_to_utc(datetime.fromisoformat(birth2_local), offset2)
    midpoint_timestamp = (dt1.timestamp() + dt2.timestamp()) / 2
    midpoint_dt = datetime.fromtimestamp(midpoint_timestamp, tz=dt1.tzinfo)
    midpoint_lat = (float(lat1) + float(lat2)) / 2
    midpoint_lon = (float(lon1) + float(lon2)) / 2
    jd = dt_to_jd(midpoint_dt)
    p = planets(jd)
    h, a = houses(jd, midpoint_lat, midpoint_lon)
    return {
        "utc": midpoint_dt.isoformat(),
...
def _natal_factors_for_relationship(
    birth_datetime: str,
    birth_offset: float,
    lat: float,
    lon: float,
) -> dict:
    utc = local_to_utc(datetime.fromisoformat(birth_datetime), birth_offset)
    jd = dt_to_jd(utc)
    p = planets(jd)
    _, a = houses(jd, lat, lon)
    result = {name: data["longitude"] for name, data in p.items()}
    result.update({"ASC": a["ASC"], "MC": a["MC"]})
    return result


def relationship_analysis(
    birth1: str, offset1: float, lat1: float, lon1: float,
    birth2: str, offset2: float, lat2: float, lon2: float,
    max_orb: float = 4.0,
) -> dict:
    f1 = _natal_factors_for_relationship(birth1, offset1, lat1, lon1)
    f2 = _natal_factors_for_relationship(birth2, offset2, lat2, lon2)
    rows = _relationship_rows(f1, f2, max_orb)

    categories = {
        "العاطفي": _category_score(rows, "emotional"),
        "العقلي": _category_score(rows, "mental"),
        "الانجذاب": _category_score(rows, "attraction"),
        "الاستقرار": _category_score(rows, "stability"),
    }
    overall = round(sum(categories.values()) / len(categories), 1)
    positive = sum(1 for r in rows if r["score"] > 0)
    challenging = sum(1 for r in rows if r["score"] < 0)
    ...
    return {
        ...
        "rows": rows,
        "composite": _composite_chart(f1, f2),
        "davison": _davison_chart(
            birth1, offset1, lat1, lon1,
            birth2, offset2, lat2, lon2,
        ),
    }


def relationship_aspects_table(rows: list) -> str:
    selected = rows[:16]
    body = []
    for row in selected:
        score = float(row["score"])
        tone = "مساند" if score > 0 else "متحدٍ"
        body.append(
            f"<tr><td>{html.escape(AR_PLANETS.get(row['factor1'], row['factor1']))}</td>"
            f"<td>{html.escape(ASPECT_AR.get(row['aspect'], row['aspect']))}</td>"
            f"<td>{html.escape(AR_PLANETS.get(row['factor2'], row['factor2']))}</td>"
            f"<td>{float(row['orb']):.2f}°</td><td>{tone}</td>"
            f"<td>{score:+.2f}</td></tr>"
        )
    return table_wrap(
        "<tr><th>الطرف الأول</th><th>الزاوية</th><th>الطرف الثاني</th>"
        "<th>الأورب</th><th>النبرة</th><th>النقاط</th></tr>",
        "".join(body),
    )



def format_position_local(lon: float) -> str:
    signs = [
        "الحمل","الثور","الجوزاء","السرطان","الأسد","العذراء",
        "الميزان","العقرب","القوس","الجدي","الدلو","الحوت"
    ]
    lon = float(lon) % 360.0
    sign_index = int(lon // 30)
    within = lon - sign_index * 30
    degree = int(within)
    minute = int(round((within - degree) * 60))
    if minute == 60:
        degree += 1
        minute = 0
    if degree == 30:
...
def simple_positions_table(title: str, positions: dict) -> str:
    rows = []
    for name in ("Sun", "Moon", "Mercury", "Venus", "Mars", "Jupiter", "Saturn", "ASC", "MC"):
        if name not in positions:
            continue
        rows.append(
            f"<tr><td>{html.escape(AR_PLANETS.get(name, name))}</td>"
            f"<td>{format_position_local(float(positions[name]))}</td></tr>"
        )
    return f"<h3>{html.escape(title)}</h3>" + table_wrap(
        "<tr><th>العامل</th><th>الموقع</th></tr>", "".join(rows)
    )
...
<title>تحليل العلاقات</title>
<style>
:root{--bg:#0e1117;--panel:#171c24;--panel2:#11161d;--border:#2c3440;--text:#f4f6f8;--muted:#aeb8c6;--accent:#7c5cff}
*{box-sizing:border-box}body{font-family:Arial,Tahoma,sans-serif;background:var(--bg);color:var(--text);margin:0}
.wrap{max-width:1080px;margin:auto;padding:16px}.card,.result-card{background:var(--panel);border:1px solid var(--border);border-radius:18px;padding:18px;margin:14px 0}
.people{display:grid;grid-template-columns:1fr 1fr;gap:14px}.grid{display:grid;grid-template-columns:repeat(3,1fr);gap:10px}
label{display:block;margin:10px 0 5px;color:#d2d8e2}input{width:100%;padding:12px;border-radius:10px;border:1px solid #3a4453;background:var(--panel2);color:white;font-size:15px}
button{width:100%;padding:15px;border:0;border-radius:12px;background:var(--accent);color:#fff;font-size:18px;font-weight:bold;margin-top:15px}
.stats{display:grid;grid-template-columns:repeat(4,1fr);gap:10px}.stats div{background:var(--panel2);padding:14px;border-radius:12px}.stats span{display:block;color:var(--muted);font-size:13px}.stats strong{font-size:24px}
...
.back{color:#cbbfff;text-decoration:none}.muted{color:var(--muted);font-size:13px}
@media(max-width:760px){.people,.grid,.stats{grid-template-columns:1fr}}
</style></head><body><div class="wrap">
<a class="back" href="/">← العودة للرئيسية</a>
<div class="card"><h1>❤️ تحليل العلاقات</h1>
<p class="muted">Synastry + Composite + Davison. المؤشرات رمزية ولا تقيس الحب أو نجاح العلاقة علميًا.</p>
<form method="post" action="/relationships-run">
<div class="people">
<section><h2>الشخص الأول</h2>
<label>الاسم</label><input name="name1" value="Person 1">
<label>تاريخ ووقت الميلاد</label><input name="birth1" value="1994-05-24T03:40:00">
<div class="grid"><div><label>UTC</label><input name="offset1" value="3"></div><div><label>خط العرض</label><input name="lat1" value="13.58"></div><div><label>خط الطول</label><input name="lon1" value="44.02"></div></div>
</section>
<section><h2>الشخص الثاني</h2>
<label>الاسم</label><input name="name2" value="Person 2">
<label>تاريخ ووقت الميلاد</label><input name="birth2" value="1990-08-22T18:00:00">
<div class="grid"><div><label>UTC</label><input name="offset2" value="3"></div><div><label>خط العرض</label><input name="lat2" value="13.58"></div><div><label>خط الطول</label><input name="lon2" value="44.02"></div></div>
</section>
</div>
<label>الأورب الأقصى للتوافق</label><input name="max_orb" value="4.0">
<button type="submit">احسب العلاقة</button>
</form></div>
__RELATION_RESULT__
</div></body></html>
"""
...
def relationships_page():
    return RELATIONSHIPS_PAGE.replace("__RELATION_RESULT__", "")


@app.post("/relationships-run", response_class=HTMLResponse)
def relationships_run(
    name1: str = Form(...), birth1: str = Form(...), offset1: float = Form(...),
    lat1: float = Form(...), lon1: float = Form(...),
    name2: str = Form(...), birth2: str = Form(...), offset2: float = Form(...),
    lat2: float = Form(...), lon2: float = Form(...),
    max_orb: float = Form(4.0),
):
    try:
        result = relationship_analysis(
            birth1, offset1, lat1, lon1,
            birth2, offset2, lat2, lon2,
            max_orb=max(1.0, min(8.0, max_orb)),
        )
        cards = "".join(
            f"<div><span>{html.escape(label)}</span><strong>{value}/100</strong></div>"
            for label, value in result["categories"].items()
        )
        composite = result["composite"]
        davison_positions = {
            **{k: v["longitude"] for k, v in result["davison"]["planets"].items()},
            "ASC": result["davison"]["angles"]["ASC"],
            "MC": result["davison"]["angles"]["MC"],
        }
        output = (
            '<section class="result-card"><h2>النتيجة العامة</h2>'
            f'<p><strong>{html.escape(name1)} × {html.escape(name2)}: {result["overall"]}/100</strong></p>'
            f'<div class="stats">{cards}</div>'
            f'<p>زوايا مساندة: {result["positive"]} — زوايا متحدية: {result["challenging"]}</p>'
            '<p class="muted">الرقم تلخيص هندسي للزوايا، وليس نسبة نجاح أو استمرار.</p></section>'
            '<section class="result-card"><h2>أقوى زوايا Synastry</h2>'
            f'{relationship_aspects_table(result["rows"])}</section>'
            '<section class="result-card"><h2>خريطة العلاقة Composite</h2>'
            f'{simple_positions_table("المواضع الوسطية", composite)}</section>'
            '<section class="result-card"><h2>خريطة Davison</h2>'
            f'<p class="muted">الوقت الوسطي UTC: {html.escape(result["davison"]["utc"])} — '
            f'الإحداثيات الوسطية: {result["davison"]["lat"]}, {result["davison"]["lon"]}</p>'
            f'{simple_positions_table("مواضع Davison", davison_positions)}</section>'
        )
    except Exception as exc:
        output = f'<section class="result-card"><h2>حدث خطأ</h2><pre>{html.escape(str(exc))}</pre></section>'
    return RELATIONSHIPS_PAGE.replace("__RELATION_RESULT__", output)


def monthly_week_score(result: dict) -> dict:
    profile = advanced_person_profile(result)
    base = profile["base"]
    return {
        "score": profile["overall"],
        "confidence": base["confidence"],
        "supportive": base["supportive"],
        "hard": base["hard"],
        "top": base["top"][:5],
        "angular": profile["angular"]["scores"],
        "confluence": profile["confluence"]["overall"],
    }


def monthly_theme_scores(result: dict) -> dict:
    rows = []
    rows.extend(clean_analysis_aspects(result["transits"]["aspects_to_natal"], "العبور"))
    rows.extend(clean_analysis_aspects(result["solar_return"]["aspects_to_natal"], "العودة الشمسية"))
    rows.extend(clean_analysis_aspects(result["secondary_progressions"]["aspects_to_natal"], "التقدمات الثانوية"))

    themes = {
        "العمل والمهنة": {"MC", "House 10", "Sun", "Saturn", "Jupiter", "Mars"},
        "المال والموارد": {"House 2", "House 8", "Venus", "Jupiter", "Saturn"},
        "العلاقات": {"House 5", "House 7", "Venus", "Moon", "Mars"},
        "التواصل والقرارات": {"Mercury", "House 3", "House 9", "Sun"},
        "الطاقة والراحة": {"Sun", "Moon", "Mars", "Saturn", "House 6", "House 12"},
    }
    output = {}
    for label, factors_set in themes.items():
        relevant = [
            r for r in rows
            if str(r["moving_factor"]) in factors_set or str(r["natal_factor"]) in factors_set
        ]
        net = 0.0
        for r in relevant:
            polarity = _aspect_polarity(str(r["aspect"]))
            net += polarity * _orb_precision(float(r.get("orb", 2.0)))
        score = 50 + 40 * math.tanh(net / max(5.0, len(relevant) ** 0.5 * 3.0))
        output[label] = round(max(5.0, min(95.0, score)), 1)
    return output


def monthly_top_table(items: list) -> str:
    rows = []
    for item in items[:10]:
        moving = AR_PLANETS.get(str(item["moving_factor"]), str(item["moving_factor"]))
        natal = AR_PLANETS.get(str(item["natal_factor"]), str(item["natal_factor"]))
        aspect = ASPECT_AR.get(str(item["aspect"]), str(item["aspect"]))
        rows.append(
            f"<tr><td>{html.escape(item.get('_source',''))}</td>"
            f"<td>{html.escape(moving)} {html.escape(aspect)} {html.escape(natal)}</td>"
            f"<td>{float(item.get('orb', 0)):.2f}°</td></tr>"
        )
    return table_wrap("<tr><th>المصدر</th><th>الإشارة</th><th>الأورب</th></tr>", "".join(rows))


MONTHLY_PAGE = """
<!doctype html><html lang="ar" dir="rtl"><head>
<meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">
<title>التحليل الشهري</title>
<style>
:root{--bg:#0e1117;--panel:#171c24;--panel2:#11161d;--border:#2c3440;--text:#f4f6f8;--muted:#aeb8c6;--accent:#7c5cff}
*{box-sizing:border-box}body{font-family:Arial,Tahoma,sans-serif;background:var(--bg);color:var(--text);margin:0}.wrap{max-width:1040px;margin:auto;padding:16px}
.card,.result-card{background:var(--panel);border:1px solid var(--border);border-radius:18px;padding:18px;margin:14px 0}
.grid{display:grid;grid-template-columns:repeat(3,1fr);gap:10px}.weeks{display:grid;grid-template-columns:repeat(2,1fr);gap:12px}
label{display:block;margin:10px 0 5px;color:#d2d8e2}input{width:100%;padding:12px;border-radius:10px;border:1px solid #3a4453;background:var(--panel2);color:white;font-size:15px}
button{width:100%;padding:15px;border:0;border-radius:12px;background:var(--accent);color:#fff;font-size:18px;font-weight:bold;margin-top:15px}
.week{background:var(--panel2);border:1px solid var(--border);border-radius:14px;padding:14px}.week strong{font-size:23px}.week span{color:var(--muted)}
.stats{display:grid;grid-template-columns:repeat(5,1fr);gap:10px}.stats div{background:var(--panel2);padding:14px;border-radius:12px}.stats span{display:block;color:var(--muted);font-size:13px}.stats strong{font-size:22px}
.table-wrap{overflow-x:auto;border:1px solid var(--border);border-radius:12px}table{width:100%;border-collapse:collapse;min-width:620px;background:var(--panel2)}th,td{padding:11px;border-bottom:1px solid var(--border);text-align:right}th{background:#191f29}
.back{color:#cbbfff;text-decoration:none}.muted{color:var(--muted);font-size:13px}
@media(max-width:760px){.grid,.weeks,.stats{grid-template-columns:1fr}}
</style></head><body><div class="wrap">
<a class="back" href="/">← العودة للرئيسية</a>
<div class="card"><h1>🗓️ التحليل الشهري المتقدم</h1>
<p class="muted">يقسم الشهر إلى أسابيع ويدمج العبور والعودة الشمسية والتقدمات والتلاقي والقوة الزاوية.</p>
<form method="post" action="/monthly-run">
<label>الاسم</label><input name="name" value="Abdulhaq">
<label>تاريخ ووقت الميلاد</label><input name="birth_datetime" value="1994-05-24T03:40:00">
<div class="grid"><div><label>UTC الميلاد</label><input name="birth_offset" value="3"></div><div><label>خط عرض الميلاد</label><input name="birth_lat" value="13.58"></div><div><label>خط طول الميلاد</label><input name="birth_lon" value="44.02"></div></div>
<div class="grid"><div><label>السنة</label><input name="year" value="2026"></div><div><label>الشهر</label><input name="month" value="7"></div><div><label>وقت القياس اليومي</label><input name="event_time" value="12:00:00"></div></div>
<div class="grid"><div><label>UTC مكان التحليل</label><input name="event_offset" value="3"></div><div><label>خط عرض المكان</label><input name="event_lat" value="15.37"></div><div><label>خط طول المكان</label><input name="event_lon" value="44.19"></div></div>
<div class="grid"><div><label>خط عرض العودة</label><input name="sr_lat" value="15.37"></div><div><label>خط طول العودة</label><input name="sr_lon" value="44.19"></div><div><label>الأورب</label><input name="max_orb" value="2.0"></div></div>
<button type="submit">أنشئ التحليل الشهري</button>
</form></div>
__MONTHLY_RESULT__
</div></body></html>
"""
...
def monthly_page():
    return MONTHLY_PAGE.replace("__MONTHLY_RESULT__", "")


@app.post("/monthly-run", response_class=HTMLResponse)
def monthly_run(
    name: str = Form(...), birth_datetime: str = Form(...),
    birth_offset: float = Form(...), birth_lat: float = Form(...), birth_lon: float = Form(...),
    year: int = Form(...), month: int = Form(...), event_time: str = Form("12:00:00"),
    event_offset: float = Form(...), event_lat: float = Form(...), event_lon: float = Form(...),
    sr_lat: float = Form(...), sr_lon: float = Form(...), max_orb: float = Form(2.0),
):
    try:
        if not 1 <= month <= 12:
            raise ValueError("الشهر يجب أن يكون بين 1 و12.")
        first = datetime(year, month, 1)
        next_month = datetime(year + (month == 12), 1 if month == 12 else month + 1, 1)
        last = next_month - timedelta(days=1)

        sample_days = sorted(set([
            1, min(8, last.day), min(15, last.day),
            min(22, last.day), last.day
        ]))
        weekly = []
        all_top = []
        theme_accumulator = []
        ...
        for day in sample_days:
            event_dt = datetime.fromisoformat(
                f"{year:04d}-{month:02d}-{day:02d}T{event_time}"
            )
            payload = {
                "name": name,
                "birth_datetime": birth_datetime,
            ...
            info = monthly_week_score(result)
            themes = monthly_theme_scores(result)
            weekly.append((event_dt, info, themes))
            all_top.extend(info["top"])
            theme_accumulator.append(themes)

        avg_score = round(sum(x[1]["score"] for x in weekly) / len(weekly), 1)
        avg_confidence = round(sum(x[1]["confidence"] for x in weekly) / len(weekly), 1)
        best = max(weekly, key=lambda x: x[1]["score"])
        hardest = min(weekly, key=lambda x: x[1]["score"])

        theme_labels = theme_accumulator[0].keys()
        theme_avg = {
            label: round(sum(t[label] for t in theme_accumulator) / len(theme_accumulator), 1)
            for label in theme_labels
        }

        week_cards = []
        for dt, info, themes in weekly:
            theme_text = " — ".join(
                f"{label}: {value}" for label, value in sorted(themes.items(), key=lambda x: -x[1])[:2]
            )
            week_cards.append(
                '<article class="week">'
                f'<h3>{dt.strftime("%Y-%m-%d")}</h3><strong>{info["score"]}/100</strong>'
                f'<p><span>التلاقي:</span> {info["confluence"]}/100 — '
                f'<span>التماسك:</span> {info["confidence"]}%</p>'
                f'<p class="muted">{html.escape(theme_text)}</p></article>'
            )

        stats = "".join(
            f"<div><span>{html.escape(label)}</span><strong>{value}/100</strong></div>"
            for label, value in theme_avg.items()
        )

        unique_top = sorted(
            all_top,
            key=lambda a: (-abs(float(a.get("_weighted", 0))), float(a.get("orb", 9)))
        )[:10]

        output = (
            '<section class="result-card"><h2>خلاصة الشهر</h2>'
            '<div class="stats">'
            f'<div><span>المؤشر العام</span><strong>{avg_score}/100</strong></div>'
            f'<div><span>تماسك البيانات</span><strong>{avg_confidence}%</strong></div>'
            f'<div><span>أفضل نقطة قياس</span><strong>{best[0].strftime("%m-%d")}</strong></div>'
            f'<div><span>أكثر نقطة ضغطًا</span><strong>{hardest[0].strftime("%m-%d")}</strong></div>'
            f'<div><span>عدد العينات</span><strong>{len(weekly)}</strong></div>'
            '</div><p class="muted">نقاط القياس تمثل بداية كل أسبوع تقريبًا ونهاية الشهر، وليست توقعًا يوميًا كاملًا.</p></section>'
            '<section class="result-card"><h2>مجالات الشهر</h2>'
            f'<div class="stats">{stats}</div></section>'
            '<section class="result-card"><h2>الخط الزمني</h2>'
            f'<div class="weeks">{"".join(week_cards)}</div></section>'
            '<section class="result-card"><h2>أقوى الإشارات خلال الشهر</h2>'
            f'{monthly_top_table(unique_top)}</section>'
            '<p class="muted">التحليل رمزي وتجريبي، ولا يستخدم لتشخيص الصحة أو ضمان أحداث مالية أو عاطفية.</p>'
        )
    except Exception as exc:
        output = f'<section class="result-card"><h2>حدث خطأ</h2><pre>{html.escape(str(exc))}</pre></section>'
    return MONTHLY_PAGE.replace("__MONTHLY_RESULT__", output)
